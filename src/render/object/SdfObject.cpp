#include <limits>
#include "SdfObject.hpp"

constexpr auto max_try_matching = 256;

SdfObject::SdfObject(Eigen::Vector3f position) : Object(position) {};

bool SdfObject::intersect_ray(const Ray& ray) const {
    return intersect_ray(ray, 0.0f, std::numeric_limits<float>::infinity());
}

bool SdfObject::intersect_ray(const Ray& ray, const float& tMin, const float& tMax) const
{
    // Ray Matching Algorithm
    auto t = tMin;
    for (int i = 0; i < max_try_matching && ray.is_valid(t) && t < tMax; i++) {
        auto dis = sdf(ray(t));
        if (dis < std::numeric_limits<float>::epsilon()) {
            return true;
        }
        t += dis;
    }

    return false;
}
